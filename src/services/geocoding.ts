...
// ✅ DUAL PROVIDER: Google Maps (primary) + Geoapify (fallback)
// Note: Using browser geolocation API only (no Capacitor in web environment)

import { shouldUseGoogleMaps } from '../config/maps';
import { searchGooglePlaces, reverseGeocodeGoogle } from './googleMaps';
...
export async function reverseGeocode(
  ...
  if (shouldUseGoogleMaps()) {
    console.log('🗺️ [ReverseGeocode] Using Google Maps (primary)');
    try {
      const googleResult = await reverseGeocodeGoogle(lat, lng);
      if (googleResult) {
        console.log('✅ [ReverseGeocode] Google Maps succeeded');
        return googleResult;
      }
    } catch (error) {
      console.warn('⚠️ [ReverseGeocode] Google Maps failed, falling back to Geoapify:', error);
    }
  }
  
  // ✅ Fallback to Geoapify
  console.log('🗺️ [ReverseGeocode] Using Geoapify (fallback)');
  try {
    await rateLimit();
    ...
    console.log('✅ [ReverseGeocode] Geoapify succeeded');
    return {
    ...
    console.error('❌ [ReverseGeocode] Both providers failed:', error);
    return null;
  }
}
 ...
 * ✅ DUAL PROVIDER: Google Places (primary) + Geoapify (fallback)
 */
export async function searchLocations(
  ...
  if (shouldUseGoogleMaps()) {
    console.log('🗺️ [SearchLocations] Using Google Places (primary)');
    try {
      const googleResults = await searchGooglePlaces(query, limit);
      if (googleResults.length > 0) {
        console.log('✅ [SearchLocations] Google Places succeeded:', googleResults.length, 'results');
        return googleResults;
      }
    } catch (error) {
      console.warn('⚠️ [SearchLocations] Google Places failed, falling back to Geoapify:', error);
    }
  }
  
  // ✅ Fallback to Geoapify
  console.log('🗺️ [SearchLocations] Using Geoapify (fallback)');
  try {
    await rateLimit();
...
/**
 * Get current GPS position with fresh coordinates (no cache)
 * Uses a double-call strategy to ensure fresh GPS lock for Browser API
 */
async function getCurrentPosition(): Promise<GeolocationPosition> {
  // Note: Capacitor is not available in web environment, using browser API only
  
  // ✅ Browser geolocation with FRESH GPS strategy
  console.log('📍 [getCurrentPosition] Using browser geolocation API with fresh GPS...');
  ...
  return new Promise((resolve, reject) => {
    ...
    // ✅ STRATEGY: Get position, check if fresh, if stale get again
    navigator.geolocation.getCurrentPosition(
      async (firstPosition) => {
        const now = Date.now();
        const positionAge = now - firstPosition.timestamp;
        
        console.log('📍 [getCurrentPosition] First position received:', {
          lat: firstPosition.coords.latitude,
          lng: firstPosition.coords.longitude,
          accuracy: Math.round(firstPosition.coords.accuracy) + 'm',
          timestamp: new Date(firstPosition.timestamp).toISOString(),
          age: Math.round(positionAge / 1000) + 's ago'
        });
        
        // ✅ If position is stale (>5s) OR has poor accuracy (>10km), get fresh GPS lock
        if (positionAge > 5000 || firstPosition.coords.accuracy > 10000) {
          if (positionAge > 5000) {
            console.log('⚠️ [getCurrentPosition] Position is stale (>5s old), requesting fresh GPS lock...');
          } else {
            console.log('⚠️ [getCurrentPosition] Position has poor accuracy (>10km), requesting fresh GPS lock...');
          }
          
          // Wait a moment for GPS to refresh, then try again
          setTimeout(() => {
            navigator.geolocation.getCurrentPosition(
              (freshPosition) => {
                const freshAge = Date.now() - freshPosition.timestamp;
                console.log('✅ [getCurrentPosition] Fresh position obtained:', {
                  lat: freshPosition.coords.latitude,
                  lng: freshPosition.coords.longitude,
                  accuracy: Math.round(freshPosition.coords.accuracy) + 'm',
                  timestamp: new Date(freshPosition.timestamp).toISOString(),
                  age: Math.round(freshAge / 1000) + 's ago'
                });
                resolve(freshPosition);
              },
              (error) => {
                console.log('⚠️ [getCurrentPosition] Fresh position failed, using first position');
                resolve(firstPosition); // Fallback to first position
              },
              options
            );
          }, 500); // Small delay to let GPS refresh
        } else {
          // Position is fresh and accurate, use it
          console.log('✅ [getCurrentPosition] Position is fresh and accurate, using it');
      ...
      (error) => {
        ...
        // Only log permission denied as a warning (not an error) since it's user choice
        if (error.code === error.PERMISSION_DENIED) {
          console.log('ℹ️ [getCurrentPosition] Location permission not granted - user can use search instead');
        } else if (error.code === error.POSITION_UNAVAILABLE) {
          console.log('ℹ️ [getCurrentPosition] Location unavailable - user can use search instead');
        } else if (error.code === error.TIMEOUT) {
          console.log('ℹ️ [getCurrentPosition] Location timeout - user can use search instead');
        }
...
export async function detectUserLocation(): Promise<GeocodedAddress | null> {
  try {
    ...
    // Get position - browser should give us fresh coordinates with maximumAge: 0
    ...
    // If accuracy is poor (>100m), warn the user but continue
    if (accuracy > 100) {
      console.warn(`⚠️ Low GPS accuracy: ${Math.round(accuracy)}m - position might not be precise`);
    }
...
// =====================================================
// DUAL-PROVIDER WRAPPER FUNCTIONS
// Try Google Maps first, fallback to Geoapify
// =====================================================

/**
 * Search locations - DUAL PROVIDER
 * Tries Google Places first, falls back to Geoapify
 */
export async function searchLocationsWithFallback(
  ...
  if (shouldUseGoogleMaps()) {
    try {
      console.log('🗺️ Using Google Places for search');
      const results = await searchGooglePlaces(query, limit);
      if (results.length > 0) {
        return results;
      }
      console.log('⚠️ Google Places returned no results, falling back to Geoapify');
    } catch (error) {
      console.warn('⚠️ Google Places search failed, falling back to Geoapify:', error);
    }
  }
  
  // Fallback to Geoapify (always available)
  console.log('🗺️ Using Geoapify for search');
  return searchLocations(query, limit);
}

/**
 * Reverse geocode - DUAL PROVIDER
 * Tries Google Geocoding first, falls back to Geoapify
 */
export async function reverseGeocodeWithFallback(
  lat: number,
  lng: number
): Promise<GeocodedAddress | null> {
  // Try Google Geocoding first (if enabled)
  if (shouldUseGoogleMaps()) {
    try {
      console.log('🗺️ Using Google Geocoding for reverse geocode');
      const result = await reverseGeocodeGoogle(lat, lng);
      if (result) {
        return result;
      }
      console.log('️ Google Geocoding returned no result, falling back to Geoapify');
    } catch (error) {
      console.warn('⚠️ Google Geocoding failed, falling back to Geoapify:', error);
    }
  }
  
  // Fallback to Geoapify (always available)
  console.log('🗺️ Using Geoapify for reverse geocode');
