...
// ✅ Import Capacitor Geolocation for mobile support
...
/**
 * Get current GPS position from browser OR Capacitor (mobile)
 * ✅ FIXED: Use Capacitor Geolocation plugin on mobile devices for better GPS accuracy
 */
export async function getCurrentPosition(): Promise<GeolocationPosition> {
  // Check if we're running in Capacitor (Android/iOS app)
  const isNativePlatform = Capacitor.isNativePlatform();
  
  console.log('📍 [getCurrentPosition] Platform:', isNativePlatform ? 'Capacitor (Mobile)' : 'Web Browser');
  
  if (isNativePlatform) {
    // ✅ Use Capacitor Geolocation plugin for mobile
    try {
      // Try to import Capacitor Geolocation dynamically (only if installed)
      let Geolocation: any;
      ...
      try {
        ...
        console.warn('⚠️ [getCurrentPosition] @capacitor/geolocation not installed, falling back to browser API');
        // Fall through to browser API below
        throw new Error('Capacitor Geolocation not available');
      }
      
      console.log('📍 [getCurrentPosition] Requesting Capacitor location...');
      
      // Request permissions first
      const permissions = await Geolocation.checkPermissions();
      console.log('📍 [getCurrentPosition] Current permissions:', permissions.location);
      
      if (permissions.location !== 'granted') {
        console.log('📍 [getCurrentPosition] Requesting permissions...');
        const requestResult = await Geolocation.requestPermissions();
        console.log('📍 [getCurrentPosition] Permission result:', requestResult.location);
      ...
      console.log('📍 [getCurrentPosition] Getting position from Capacitor...');
      ...
      console.log('✅ [getCurrentPosition] Capacitor position obtained:', {
      ...
      console.log('⚠️ [getCurrentPosition] Capacitor geolocation failed, using browser API fallback:', error.message);
      // Fall through to browser API
    }
  }
  
  // ✅ Fallback to browser geolocation API for web (or if Capacitor fails)
  console.log('📍 [getCurrentPosition] Using browser geolocation API...');
  ...
  return new Promise((resolve, reject) => {
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        console.log('✅ [getCurrentPosition] Browser position obtained:', {
          lat: position.coords.latitude,
          lng: position.coords.longitude,
          accuracy: position.coords.accuracy
        });
      ...
      (error) => {
        ...
        console.error('❌ [getCurrentPosition] Browser geolocation error:', errorMessage);
