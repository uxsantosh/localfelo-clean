...
export async function getTasks(filters?: TaskFilters): Promise<{ tasks: Task[]; totalCount: number }> {
  try {
    ...
    let countQuery = supabase
      ...
      // ✅ FIX: Show ALL active tasks (not just open/negotiating)
      // Show: NULL, open, negotiating, accepted, in_progress
      // Hide: completed, closed
      .or('status.is.null,status.eq.open,status.eq.negotiating,status.eq.accepted,status.eq.in_progress');
    ...
    const tasks: Task[] = data.map(task => {
      ...
      return {
        ...
        images: task.images || [], // ✅ ADD: Include images array
  ...
  try {
    ...
    if (data.user_id) {
      ...
      if (profileData && profileData.name) {
        ...
        console.log('📞 [TaskService] User phone fetched:', userPhone ? 'Available' : 'Not available');
      }
    }
    ...
    return {
      ...
      userPhone: userPhone, // NEW: Include user phone
      ...
      images: data.images || [], // ✅ ADD: Include images array
      // ❌ REMOVED: These fields don't exist in database
      // helperCompleted: false,
      // creatorCompleted: false,
      distance,
...
export async function createTask(task: {
  ...
  whatsapp?: string;
  hasWhatsapp?: boolean;
  helperCategory?: string; // NEW: Helper category from frontend
  ...
  try {
    ...
    // ✅ ROLE-BASED SYSTEM: Map helper_category to role_id
    let roleId = null;
    if (task.helperCategory) {
      try {
        ...
        if (roleResult.success && roleResult.roleId) {
          ...
          console.log(`✅ Mapped task to role: ${task.helperCategory} → ${roleResult.roleId}`);
        }
      } catch (roleError) {
        console.warn('⚠️ Role mapping failed, continuing without role:', roleError);
      }
    }
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        status: 'open',
        role_id: roleId, // ✅ NEW: Save role_id
        helper_category: task.helperCategory || null, // ✅ NEW: Save helper_category
      })
      .select()
      .single();
    ...
    console.log('✅ [TaskService] Task created successfully with role_id:', roleId);

    // 🔔 IMMEDIATE NOTIFICATIONS: Notify matching professionals AND helpers
    try {
      await notifyMatchingProvidersForTask(
        data.id,
        task.title,
        task.description,
        roleId,
  ...
  try {
    ...
    return data.map((task: any) => {
      ...
      return {
        ...
        images: task.images || [], // ✅ FIX: Include images for edit mode
        detected_category: task.detected_category, // ✅ FIX: Include detected_category for edit mode
        subcategory: task.subcategory, // ✅ FIX: Include subcategory for edit mode
      };
    });
  } catch (error) {
...
/**
 * Notify matching providers for a new task
 * @param taskId - Task ID
 * @param title - Task title
 * @param description - Task description
 * @param roleId - Role ID (mapped from helper category)
 * @param cityId - City ID
 * @param areaId - Area ID
 * @param userId - User ID (task creator)
 */
async function notifyMatchingProvidersForTask(
  taskId: string,
  title: string,
  description: string,
  roleId: string | null,
  ...
  try {
    console.log('🔔 [Task Notifications] Finding matching providers...');
      ...
      try {
        ...
        const { error: notifError } = await supabase
          ...
          .insert({
            ...
            title: `New Task Match: ${title}`,
            message: `${requesterName} posted a task you can help with. Tap to view details.`,
        ...
        if (!notifError) {
          ...
          if (provider.whatsapp) {
            sendWhatsAppNotification({
              ...
              templateName: 'task_match',
              variables: {
                provider_name: provider.name,
                requester_name: requesterName,
                task_title: title,
                task_preview: description?.substring(0, 100) || '',
                task_link: `${window.location.origin}/tasks/${taskId}`
              },
