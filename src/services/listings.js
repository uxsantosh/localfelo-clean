...
async function transformListing(dbListing) {
  ...
  let userPhone = ''; // NEW: Fetch phone from profile
  if (!userName || userName.trim() === '') {
    if (dbListing.owner_token) {
      try {
        const { data: profileData } = await supabase
          .from('profiles')
          .select('name, phone') // NEW: Also fetch phone
          .eq('id', dbListing.owner_token)
          .single();
        
        if (profileData && profileData.name) {
          userName = profileData.name;
          userPhone = profileData.phone || ''; // NEW: Get phone from profile
          console.log('📞 [ListingService] User phone fetched:', userPhone ? 'Available' : 'Not available');
        } else {
          userName = 'Local User';
        }
      } catch (error) {
  ...
  } else if (dbListing.owner_token) {
    // NEW: Even if userName is present, we need to fetch phone
    try {
      const { data: profileData } = await supabase
        .from('profiles')
        .select('phone')
      ...
      console.log('[transformListing] Could not fetch user phone');
    }
  }
  ...
  return {
    ...
    phone: userPhone || dbListing.owner_phone, // NEW: Use phone from profile, fallback to db
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        subcategory_id: payload.subcategoryId || null, // ✅ FIX 1: Store subcategory
    ...
    // ✅ FIX 2: Send notifications to matching wishes
    try {
      await sendWishMatchNotifications(data);
    } catch (notifError) {
      console.error('[Service] Error sending wish notifications:', notifError);
...
/**
 * HARD DELETE a listing permanently (admin/user permanent delete)
 * @param {string} listingId - Listing ID to permanently delete
 * @param {string} clientToken - Client token for verification (optional for admin)
 * @returns {Promise<void>}
 */
export async function deleteListingPermanently(listingId, clientToken) {
  console.log('[Service] deleteListingPermanently called with id:', listingId);
  ...
  try {
    if (clientToken) {
      ...
      if (error) {
        console.error('[Service] Error permanently deleting listing:', error);
        throw error;
      }
    } else {
      ...
      if (error) {
        console.error('[Service] Error permanently deleting listing (admin):', error);
        throw error;
      }
    }
    
    console.log('[Service] ✅ Listing permanently deleted from database');
  } catch (error) {
    console.error('[Service] deleteListingPermanently error:', error);
    throw error;
  }
}
...
/**
 * ✅ FIX 2: Send notifications to wishes that match the new listing
 * @param {Object} listing - The newly created listing
 * @returns {Promise<void>}
 */
async function sendWishMatchNotifications(listing) {
  console.log('🔔 [WISHES MATCHING] Checking for matching wishes for listing:', listing.id);
  console.log('   Listing category:', listing.category_slug);
  console.log('   Listing subcategory:', listing.subcategory_id);
  
  try {
    // ✅ FIX 6: Import configurable settings
    ...
    const { data: wishes, error } = await supabase
      ...
      .select('*')
      .eq('is_hidden', false)
      .neq('user_id', listing.user_id) // Don't notify the listing creator about their own listing
      .in('status', ['open', 'negotiating']); // Only active wishes
    ...
    if (!wishes || wishes.length === 0) {
      console.log('  ℹ️ No active wishes found');
      return;
    }
    
    console.log(`  ✅ Found ${wishes.length} active wish(es)`);
    
    // ✅ NEW: Filter by CATEGORY + SUBCATEGORY + LOCATION
    const matchingWishes = wishes.filter(wish => {
      // STEP 1: Check if wish has location (required for distance calculation)
      if (!wish.latitude || !wish.longitude || !listing.latitude || !listing.longitude) {
        return false;
      }
      
      // STEP 2: Calculate distance (must be within 5km)
      const distance = calculateDistance(
        wish.latitude,
        wish.longitude,
        listing.latitude,
      ...
      // Attach distance to wish for sorting later (FIX 3)
      wish._distance = distance;
      
      if (distance > MATCHING_RADIUS_KM) {
        console.log(`  ❌ Too far for wish "${wish.title}": ${distance.toFixed(1)}km > ${MATCHING_RADIUS_KM}km`);
        return false; // Too far
      }
      
      // STEP 3: Check category + subcategory matching
      // NEW SYSTEM: Use category_ids and subcategory_ids arrays
      if (wish.category_ids && Array.isArray(wish.category_ids)) {
        // Check if listing category matches any wish category
        const categoryMatch = wish.category_ids.includes(listing.category_slug);
        
        if (!categoryMatch) {
          console.log(`  ❌ Category mismatch for wish "${wish.title}": want [${wish.category_ids.join(', ')}], got ${listing.category_slug}`);
          return false;
        }
        
        // If listing has subcategory, check subcategory match
        if (listing.subcategory_id) {
          if (!wish.subcategory_ids || !Array.isArray(wish.subcategory_ids)) {
            console.log(`  ❌ Subcategory mismatch for wish "${wish.title}": wish has no subcategories but listing does`);
            return false;
          }
          
          const subcategoryMatch = wish.subcategory_ids.includes(listing.subcategory_id);
          if (!subcategoryMatch) {
            console.log(`  ❌ Subcategory mismatch for wish "${wish.title}": want [${wish.subcategory_ids.join(', ')}], got ${listing.subcategory_id}`);
            return false;
          }
        }
        
        console.log(`  ✅ MATCH FOUND: wish "${wish.title}" matches listing "${listing.title}"`);
        console.log(`     Category: ${listing.category_slug}, Subcategory: ${listing.subcategory_id}, Distance: ${distance.toFixed(1)}km`);
        return true;
      }
      
      // OLD SYSTEM FALLBACK: For wishes created before migration
      // Only match on category (no subcategory precision)
      if (wish.category_id) {
        const categoryMatch = String(wish.category_id) === listing.category_slug;
        if (categoryMatch) {
          console.log(`  ⚠️ OLD SYSTEM match for wish "${wish.title}" (no subcategory precision)`);
          return true;
        }
      }
      
      console.log(`  ❌ No match for wish "${wish.title}": no category data`);
      return false;
    });
    
    console.log(`  🎯 ${matchingWishes.length} wish(es) match (category + subcategory + location within ${MATCHING_RADIUS_KM}km)`);
    
    if (matchingWishes.length === 0) {
      console.log('  ℹ️ No matching wishes found');
      return;
    }
    ...
    // ✅ STEP 3: Anti-spam - Check for duplicate notifications (within 30 minutes)
    const duplicateWindowMs = DUPLICATE_WINDOW_MINUTES * 60 * 1000;
    const duplicateWindowAgo = new Date(Date.now() - duplicateWindowMs).toISOString();
    const userIds = matchingWishes.map(w => w.user_id);
    ...
    // Filter out users who already received notification for this listing recently
    let wishesToNotify = matchingWishes.filter(wish => {
      if (recentNotifUserIds.has(wish.user_id)) {
        console.log(`  ⏭️ Skipping duplicate notification for user ${wish.user_id}`);
        return false;
      }
      return true;
    });
    
    if (wishesToNotify.length === 0) {
      console.log('  ℹ️ All notifications were duplicates, skipping');
      return;
    }
    ...
    if (wishesToNotify.length === 0) {
      console.log('  ℹ️ All users hit rate limit, no notifications sent');
      return;
    }
    ...
    if (wishesToNotify.length > 1) {
      ...
      if (notifError) {
        ...
        console.log(`  ✅ Sent ${notifications.length} notification(s) to wish creators`);
      }
    } else {
      // Single notification
      const wish = wishesToNotify[0];
      const notification = {
        user_id: wish.user_id,
      ...
      if (notifError) {
        ...
        console.log(`  ✅ Sent 1 notification to wish creator`);
      }
    }
  } catch (error) {
    console.error('❌ Error in sendWishMatchNotifications:', error);
    // Don't throw - notifications failure shouldn't break listing creation
  }
}