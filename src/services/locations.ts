...
export async function getCitiesWithAreas() {
  ...
  if (IS_OFFLINE_MODE) {
    console.log('📍 [Locations] Using mock cities data (offline mode)');
    return MOCK_CITIES;
  }
  ...
  try {
    ...
    if (error && error.code === '42703') {
      ...
      if (fallbackResult.error) {
        console.error('[getCities] Error fetching cities:', fallbackResult.error);
...
export async function getSubAreasForArea(areaId: string) {
  // Return empty array immediately in offline/preview mode
  if (IS_OFFLINE_MODE) {
    console.log('📍 [Locations] Skipping sub-areas fetch (offline mode)');
    return [];
  }
  ...
  try {
    ...
    if (error) {
      console.error('[getCities] Error fetching sub-areas:', error);
      return [];
    }

    return data || [];
  } catch (err) {
    console.error('[getCities] Error fetching sub-areas:', err);
    return [];
  }
}