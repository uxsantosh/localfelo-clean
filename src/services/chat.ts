...
export async function getConversations(): Promise<{
  ...
  try {
    const userId = await getUserId();
    if (!userId) {
      console.log('ℹ️ [getConversations] User not authenticated - skipping fetch');
    ...
    // Log all conversations with their details
    data?.forEach((conv, index) => {
      console.log(`  [${index + 1}] ID: ${conv.id.slice(0, 8)}... Type: ${conv.listingtype || 'listing'} Title: ${conv.listing_title} LastMsg: ${conv.last_message || 'none'}`);
    });
...
export async function getMessages(
  ...
  try {
    const { data, error } = await supabase
      .from('messages')
      .select('id, conversation_id, sender_id, sender_name, sender_avatar, content, read, created_at')
      .eq('conversation_id', conversationId)
      .order('created_at', { ascending: true });

    if (error) throw error;
    
    console.log('📨 [getMessages] Loaded messages:', {
      conversationId,
      count: data?.length || 0,
      messages: data
    });
  ...
  const channel = supabase
    ...
    .on(
      ...
      (payload) => {
        ...
        // If sender_name is missing, fetch it (shouldn't happen with trigger, but safety net)
        if (!message.sender_name) {
          console.warn('⚠️ Message missing sender_name, fetching from profile...');
          supabase
            .from('profiles')
            ...
            .then(({ data: profile }) => {
              if (profile) {
                message.sender_name = profile.display_name || 'User';
                message.sender_avatar = profile.avatar_url || undefined;
              } else {
          ...
          // Message has sender_name, use it directly
          onMessage(message);
        }
      }
    )
