...
export async function requestPushPermission(): Promise<boolean> {
  try {
    ...
    if (isCapacitor) {
      ...
      // Permission is handled by PushNotifications.requestPermissions()
      // inside the hook, NOT here.
      console.log('[PushClient] Capacitor native - permission handled by PushNotifications plugin');
      return true;
    }
    ...
    console.log('[PushClient] Web notification permission result:', result);
...
export function isPushSupported(): boolean {
  try {
    // ✅ FIX: Check if running on Capacitor native (Android/iOS)
    const isCapacitor =
      typeof window !== 'undefined' &&
      (window as any).Capacitor?.isNativePlatform?.();

    if (isCapacitor) {
      // Android / iOS via Capacitor always support native push
      console.log('[PushClient] Running on Capacitor native platform - push supported');
      return true;
    }

    // Web fallback: Check if we're in a secure context (HTTPS or localhost)
...
export function getPushPermissionStatus(): 'granted' | 'denied' | 'default' | 'unavailable' {
  try {
    ...
    if (isCapacitor) {
      // Native permission handled by PushNotifications plugin
      // Return 'default' to allow permission request flow to proceed
      console.log('[PushClient] Capacitor native - returning default permission status');
      return 'default';
    }
