...
export async function debugProfileIssue() {
  ...
  if (allListings && allListings.length > 0) {
    ...
    if (mismatchedListings.length > 0) {
      ...
      console.log('   Run: window.fixOwnerTokens() to update them.');
    } else {
...
export async function fixOwnerTokens() {
...
if (typeof window !== 'undefined') {
  (window as any).debugProfileIssue = debugProfileIssue;
  (window as any).fixOwnerTokens = fixOwnerTokens;
}
