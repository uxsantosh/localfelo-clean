-- =====================================================
-- DATA INTELLIGENCE DASHBOARD - COMPLETE FIX
-- Now includes the REAL 46 Service Categories
-- =====================================================
...
-- =====================================================
-- STEP 2: Create Service Categories Table (46 Categories)
-- =====================================================
...
-- View 2: Service Category Performance (46 Categories - REAL DATA)
...
SELECT 
  ...
  (SELECT COUNT(DISTINCT user_id) FROM helper_preferences 
   WHERE sc.id = ANY(selected_categories)) as helpers_available
...
-- View 3: Wish Category Stats (using old categories table)
...
-- View 4: Marketplace Category Stats (using old categories table)
...
SELECT 
  ...
  -- NEW: Helper's selected categories
  (SELECT selected_categories FROM helper_preferences WHERE user_id = p.id) as helper_categories,
  (SELECT array_length(selected_categories, 1) FROM helper_preferences WHERE user_id = p.id) as num_categories
FROM profiles p
INNER JOIN tasks t ON t.accepted_by = p.id
...
BEGIN
  ...
  SELECT 
    ...
    (SELECT COUNT(*) FROM helper_preferences WHERE is_available = true)::BIGINT as total_helpers;
END;
...
BEGIN
  ...
  SELECT 
    ...
    h.avg_task_value,
    c.name as city_name,
    COALESCE(h.num_categories, 0)::INTEGER as num_categories
...
SELECT * FROM service_categories ORDER BY priority DESC, name;

-- Check service category stats
