...
export interface User {
  ...
  clientToken: string;
  profilePic?: string;
  avatar_url?: string; // NEW: Profile photo URL
  gender?: 'male' | 'female' | 'other' | null; // NEW: User gender
...
export interface Wish {
  ...
  title: string;
  description: string;
  categoryId?: string; // Old system (keep for backward compatibility)
  category_ids?: string[]; // NEW: Array of category slugs for precise matching
  subcategory_ids?: string[]; // NEW: Array of subcategory IDs for precise matching
  budgetMin?: number;
  budgetMax?: number;
  ...
  subcategoryId?: string; // OLD: Product subcategory ID (keep for backward compatibility)
  productName?: string; // Custom product name for "Other" category
}
...
export interface CreateWishData {
  title: string;
  description: string;
  categoryId: string | number; // OLD: Support both string and number for compatibility (deprecated)
  categoryIds?: string[]; // NEW: Array of category slugs for precise matching
  subcategoryIds?: string[]; // NEW: Array of subcategory IDs for precise matching
  budgetMin?: number;
  budgetMax?: number;
  urgency: 'asap' | 'today' | 'flexible';
  cityId: string | null;
  ...
  subcategoryId?: string; // OLD: Product subcategory ID (deprecated - use subcategoryIds)
  productName?: string; // Custom product name for "Other" category
}
...
export interface Task {
  ...
  userName: string;
  userAvatar?: string;
  userPhone?: string; // NEW: User's registered phone number from profile
  status: TaskStatus;
  createdAt: string;
  latitude?: number;
  longitude?: number;
  ...
  helperName?: string;
  helperAvatar?: string;
  images?: string[]; // NEW: Array of image URLs (max 3 images)
  // ❌ REMOVED: helperCompleted and creatorCompleted don't exist in database
