...
if (!isFigmaPreview) {
  ...
  fetch(supabaseUrl, { method: 'HEAD' })
    .then(() => {
      console.log('✅ Supabase URL is reachable:', supabaseUrl);
    })
    .catch((error) => {
      ...
      console.error('   1. You are offline or have poor internet connection');
      console.error('   2. DNS cannot resolve:', supabaseUrl);
      console.error('   3. Firewall/VPN is blocking Supabase');
      console.error('   4. Browser security settings blocking requests');
      console.error('');
      console.error('💡 Try:');
      console.error('   - Check your internet connection');
      console.error('   - Disable VPN/proxy');
