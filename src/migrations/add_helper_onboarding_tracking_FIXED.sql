-- FIXED: Add onboarding tracking columns to helper_preferences table
...
-- Add these preference columns if they don't exist
ALTER TABLE helper_preferences 
...
ALTER TABLE helper_preferences 
ADD COLUMN IF NOT EXISTS min_confidence_threshold INTEGER DEFAULT 60;

ALTER TABLE helper_preferences 
ADD COLUMN IF NOT EXISTS max_distance INTEGER DEFAULT 10;

ALTER TABLE helper_preferences 
ADD COLUMN IF NOT EXISTS min_budget INTEGER DEFAULT 100;

-- Add categories column (was called 'skills' in the previous code but doesn't exist)
-- We'll use TEXT[] for storing category slugs
ALTER TABLE helper_preferences 
...
COMMENT ON COLUMN helper_preferences.selected_categories IS 'Array of category slugs helper is interested in';
...
COMMENT ON COLUMN helper_preferences.show_all_tasks IS 'Whether to show ALL tasks regardless of category match';
...
-- Update existing helpers with empty categories to have smart defaults
...
SET 
  ...
  max_distance = COALESCE(max_distance, 10),
  min_budget = COALESCE(min_budget, 100)
WHERE 
  (selected_categories IS NULL OR selected_categories = '{}' OR array_length(selected_categories, 1) = 0 OR array_length(selected_categories, 1) IS NULL)
...
BEGIN
  ...
  IF v_prefs.onboarding_completed = false OR v_prefs.onboarding_completed IS NULL THEN
    RETURN true;
  END IF;
  
  -- If no categories selected, onboarding needed
  IF v_prefs.selected_categories IS NULL OR 
     v_prefs.selected_categories = '{}' OR 
     array_length(v_prefs.selected_categories, 1) = 0 OR
     array_length(v_prefs.selected_categories, 1) IS NULL THEN
    RETURN true;
  END IF;
...
BEGIN
  ...
  v_has_categories := v_prefs IS NOT NULL AND 
                      v_prefs.selected_categories IS NOT NULL AND 
                      array_length(v_prefs.selected_categories, 1) > 0;
...
-- RLS Policies for helper_preferences
DROP POLICY IF EXISTS "Users can view own helper preferences" ON helper_preferences;
CREATE POLICY "Users can view own helper preferences" 
ON helper_preferences FOR SELECT 
USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert own helper preferences" ON helper_preferences;
CREATE POLICY "Users can insert own helper preferences" 
ON helper_preferences FOR INSERT 
WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update own helper preferences" ON helper_preferences;
CREATE POLICY "Users can update own helper preferences" 
ON helper_preferences FOR UPDATE 
USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can delete own helper preferences" ON helper_preferences;
CREATE POLICY "Users can delete own helper preferences" 
