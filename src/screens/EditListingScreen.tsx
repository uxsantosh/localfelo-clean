...
export function EditListingScreen({ 
  ...
  // Form state - Initialize with listing data (with null checks)
  ...
  const [selectedCategory, setSelectedCategory] = useState(listing.categoryId ? listing.categoryId.toString() : '');
  ...
  useEffect(() => {
    ...
    if (listing?.isNegotiable !== undefined) setIsNegotiable(listing.isNegotiable);
    if (listing?.categoryId) setSelectedCategory(listing.categoryId.toString());
    if (listing?.condition) setCondition(listing.condition);
    if (listing?.address) setUserAddress(listing.address);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const categorySlug = selectedCategoryObj?.slug || listing.categorySlug;
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-3">
                <label className="flex items-center gap-3 cursor-pointer">
                  ...
                  />
                  <span className="text-sm text-black">Price is negotiable</span>
                </label>
              </div>
