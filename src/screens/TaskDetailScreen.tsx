...
export function TaskDetailScreen({ taskId, onNavigate, onBack, isLoggedIn, onLoginRequired, isAdmin, userDisplayName, showGlobalLocation, globalLocationArea, globalLocationCity, onLocationClick, onMenuClick, unreadCount, notificationCount, onNotificationClick, userCoordinates }: TaskDetailScreenProps) {
  ...
  const handleAccept = async () => {
    ...
    if (!confirm('Accept this task? The person who posted it will be notified.')) {
      return;
    }
  ...
  // Debug logging
  console.log('🎯 [TaskDetail] BUTTON RENDERING LOGIC:', {
    taskId: task.id,
    taskStatus: task.status,
    currentUserRawId: currentUser?.id,
    currentUserResolvedUUID: currentUserUUID,
  ...
  console.log('🔘 [TaskDetail] Selected button set:', buttonSet);
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      {/* 3. HELPER - OPEN: Negotiate + Accept */}
      {buttonSet === 'helper-open' && taskOwnerProfileExists && !profileCheckLoading && (
        <div className="fixed bottom-16 md:bottom-4 left-0 right-0 bg-white border-t border-border shadow-lg p-4 z-50">
          <div className="max-w-4xl mx-auto flex gap-3">
            <button
              onClick={handleNegotiate}
              className="flex-1 px-6 py-4 bg-white border-2 border-black text-black rounded-xl font-semibold hover:bg-gray-50 transition-colors flex items-center justify-center gap-2 text-base"
            >
              <MessageCircle className="w-5 h-5" />
              Negotiate
            </button>
      ...
      {buttonSet === 'helper-accepted' && (
        <div className="fixed bottom-16 md:bottom-4 left-0 right-0 bg-white border-t border-border shadow-lg p-3 z-50">
