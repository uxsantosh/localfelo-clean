...
import { Phone, Lock, Eye, EyeOff, ArrowRight, User, CheckCircle2, MessageCircle } from 'lucide-react';
...
export function PhoneAuthScreen({ onSuccess, onClose }: PhoneAuthScreenProps) {
  ...
  const handlePhoneContinue = async () => {
    ...
    try {
      ...
      const cleanPhone = formatted.clean; // e.g., "9876543210" - 10 digits
      ...
      // ✅ OPTIMIZED: Single query with OR conditions for backward compatibility
      const phoneVariants = [
        cleanPhone,        // 9063205739 (NEW format)
        `+91${cleanPhone}`, // +919063205739 (OLD format)
        `91${cleanPhone}`,  // 919063205739 (LEGACY format)
      ];

      console.log('🔍 Searching for phone in these formats:', phoneVariants);

      // Query phone column with all variants - try each format
      let profile = null;
      
      for (const variant of phoneVariants) {
        console.log('🔍 Trying phone format:', variant);
        ...
        // Also try phone_number column for extra safety
        const { data: phoneNumberMatch } = await supabase
          .from('profiles')
          .select('*')
        ...
        if (phoneNumberMatch) {
          profile = phoneNumberMatch;
          console.log('✅ Found profile with phone_number:', variant);
          break;
        }
      }
  ...
  const handlePasswordLogin = async () => {
    ...
    try {
      ...
      // Password correct - continue with login (keep loading=true)
      // ✅ REMOVED: setLoading(false) - keep it true until fully complete
      ...
      try {
        ...
        if (authError || !authData?.success) {
          ...
          if (authError?.message?.includes('Failed to send a request') || authError?.message?.includes('FunctionsFetchError')) {
            console.debug('⚠️ Edge function not available (using localStorage auth fallback)');
          } else {
        ...
        // Edge Function doesn't exist or failed - this is expected in development
        if (err?.message?.includes('Failed to send a request') || err?.message?.includes('FunctionsFetchError')) {
          console.debug('⚠️ Edge function not available (using localStorage auth fallback)');
        } else {
    ...
    try {
      ...
      if (existingProfile) {
        ...
        try {
          ...
          console.log('🔍 Edge function raw response:', { data: authData, error: authError });
          
          if (authError) {
            // Edge Function doesn't exist or failed - this is expected in development
            if (authError?.message?.includes('Failed to send a request') || authError?.message?.includes('FunctionsFetchError')) {
              console.debug('⚠️ Edge function not available (development mode)');
              throw new Error('Edge function not available. Please deploy edge functions for production use.');
            }
            console.error('❌ Edge function error object:', JSON.stringify(authError, null, 2));
          }
          
          if (authData) {
            console.log('📦 Edge function data:', JSON.stringify(authData, null, 2));
          }

          if (authError || !authData?.success) {
            const errorMessage = authData?.error || authError?.message || 'Failed to create account. Please try again.';
            console.error('❌ Failed to create user via edge function:', errorMessage);
            console.error('❌ Full error details:', { authError, authData });
          ...
          setLoading(false);
          fireConfetti();
          // ❌ REMOVED: Duplicate toast - App.tsx already shows welcome message
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="auth-input-field w-full pl-12 pr-4 py-3"
                      maxLength={10}
                      autoFocus
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    10-digit Indian WhatsApp number
                  </p>
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="auth-input-field has-right-icon w-full"
                      autoFocus
