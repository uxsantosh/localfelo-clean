...
import { 
  ...
  AlertTriangle, MessageSquare, Mail, Phone, BarChart3, TrendingUp, Store
} from 'lucide-react';
...
import { MarketplaceIntelligenceTab } from '../components/admin/MarketplaceIntelligenceTab';
import { DataIntelligenceTab } from '../components/admin/DataIntelligenceTab'; // ← NEW: Data Intelligence
import WhatsAppTestPanel from '../components/admin/WhatsAppTestPanel'; // ← NEW: WhatsApp testing
import { ProfessionalsManagementTab } from '../components/admin/ProfessionalsManagementTab'; // ← NEW: Professionals Management
import { RolesManagementTab } from '../components/admin/RolesManagementTab'; // ← NEW: Roles Management
import { RoleSyncTab } from '../components/admin/RoleSyncTab'; // ← NEW: Role Sync Tool
import { ShopsManagementTab } from '../components/admin/ShopsManagementTab'; // ← NEW: Shops Management
...
export function AdminScreen({
  ...
  const [activeTab, setActiveTab] = useState<'listings' | 'users' | 'wishes' | 'tasks' | 'reports' | 'site-settings' | 'broadcast' | 'chat-history' | 'all-chats' | 'footer-pages' | 'whatsapp-test' | 'marketplace-intelligence' | 'data-intelligence' | 'professionals' | 'roles' | 'role-sync' | 'shops'>('listings');
  ...
  const handleToggleListingVisibility = async (listingId: string, currentlyHidden: boolean) => {
    try {
      ...
      // Reload data from database to get all updated fields
      await loadListings();
  ...
  const handleDeleteListing = async (listingId: string) => {
    ...
    try {
      ...
      // Reload data from database instead of just filtering local state
      await loadListings();
  ...
  const handleEditListing = async () => {
    ...
    try {
      ...
      // Reload data from database to get all updated fields including joined data
      await loadListings();
  ...
  return (
    <div className="min-h-screen bg-white pb-20">
      ...
      <div className="page-container py-6">
        ...
        <div className="mb-6">
          <ScrollableContainer>
            <div className="flex gap-1 min-w-max bg-white p-1" style={{ borderRadius: '8px' }}>
              ...
              <button
                onClick={() => { setActiveTab('whatsapp-test'); setSearchTerm(''); }}
                className={`flex items-center gap-2 px-4 py-2.5 text-sm font-medium transition-all ${
              ...
              <button
                onClick={() => { setActiveTab('marketplace-intelligence'); setSearchTerm(''); }}
                className={`flex items-center gap-2 px-4 py-2.5 text-sm font-medium transition-all ${
                  activeTab === 'marketplace-intelligence'
                    ? 'bg-black text-white font-bold'
                    : 'text-gray-600 hover:text-black'
                }`}
                style={{ borderRadius: '6px' }}
              ...
              <button
                onClick={() => { setActiveTab('data-intelligence'); setSearchTerm(''); }}
                className={`flex items-center gap-2 px-4 py-2.5 text-sm font-medium transition-all ${
              ...
              <button
                onClick={() => { setActiveTab('role-sync'); setSearchTerm(''); }}
                className={`flex items-center gap-2 px-4 py-2.5 text-sm font-medium transition-all ${
                  activeTab === 'role-sync'
                    ? 'bg-black text-white font-bold'
                    : 'text-gray-600 hover:text-black'
        ...
        {activeTab !== 'site-settings' && activeTab !== 'wishes' && activeTab !== 'tasks' && activeTab !== 'reports' && activeTab !== 'broadcast' && activeTab !== 'chat-history' && activeTab !== 'all-chats' && activeTab !== 'footer-pages' && activeTab !== 'whatsapp-test' && activeTab !== 'marketplace-intelligence' && activeTab !== 'data-intelligence' && activeTab !== 'professionals' && activeTab !== 'roles' && activeTab !== 'role-sync' && activeTab !== 'shops' && (
        ...
        {activeTab === 'listings' ? (
          ...
          <ShopsManagementTab onNavigate={onNavigate} />
        ) : null}
      </div>
