...
import { Plus, Filter, MapPin, List, Map, Briefcase, User as UserIcon, Edit2, Trash2, Search, X, SlidersHorizontal, Sparkles, Loader2 } from 'lucide-react';
...
export function TasksScreen({
  ...
  // Load tasks - reset to page 1 when filters change
  useEffect(() => {
  ...
  const loadTasks = async (pageNum: number = 1, isRefresh: boolean = false) => {
    try {
      ...
      // DO NOT filter by city/area - show ALL tasks from everywhere
      // Only apply category, search, and status filters
      
      const filters: any = {
        categoryId: selectedCategory?.toString() || undefined,
        // REMOVED: City and area filters - we show ALL tasks everywhere
      ...
      setHasMore(tasks.length + filteredTasks.length < result.totalCount);
    } catch (error) {
              ...
              <div ref={observerTarget} className="py-4">
                ...
                {!hasMore && !loadingMore && (
                  <p className="text-center text-sm text-gray-500">
                    ✨ You've seen all tasks! {totalCount > 0 && `(${totalCount} total)`}
                  </p>
                )}
              </div>
            )}
                        ...
                        <div key={task.id} className="bg-white border border-gray-200 rounded-xl p-4 hover:shadow-md transition-shadow">
                          <div className="flex items-start gap-3">
                            ...
                            >
                              ...
                              <p className="text-sm text-gray-600 mb-2 line-clamp-2">{task.description}</p>
                              {task.price && (
                                <p className="text-sm font-medium text-black">Earn ₹{task.price.toLocaleString('en-IN')}</p>
                              )}
                            </div>
                            <div className="flex flex-col gap-2">
                              <button
