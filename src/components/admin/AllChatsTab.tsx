...
import { MessageCircle, Search, Trash2, User, Clock, ChevronRight, Download } from 'lucide-react';
...
export function AllChatsTab() {
  ...
  const loadAllChats = async () => {
    setLoading(true);
    try {
      console.log('🔍 [AllChatsTab] Starting to load all chats...');
      ...
      if (messagesError) {
        console.error('❌ [AllChatsTab] Error fetching messages:', messagesError);
        console.error('   Error details:', JSON.stringify(messagesError, null, 2));
        throw messagesError;
      }

      console.log(`📊 [AllChatsTab] Fetched ${messagesData?.length || 0} messages from database`);
