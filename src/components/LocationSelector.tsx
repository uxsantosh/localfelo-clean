...
import { LocationMapNative } from './LocationMapNative';
...
export function LocationSelector({ 
  ...
  // Initialize with currentLocation if available, otherwise India center
  useEffect(() => {
    if (currentLocation && currentLocation.latitude && currentLocation.longitude) {
      // User is editing - show their previously selected location
      console.log('🎯 [LocationSelector] Initializing with current location:', currentLocation);
      setSelectedLocation({
        latitude: currentLocation.latitude,
        longitude: currentLocation.longitude,
        address: currentLocation.city || 'Selected Location',
        city: currentLocation.city || '',
        country: 'India'
      });
    } else {
      // New selection - start at India center
      console.log('🌍 [LocationSelector] No current location, starting at India center');
      setSelectedLocation({
        latitude: 20.5937,
        longitude: 78.9629,
        address: 'India',
  ...
  const handleAutoDetect = async () => {
    ...
    try {
      ...
      if (location) {
        ...
        console.log('🎯 [LocationSelector] Setting map to coordinates:', {
          lat: location.latitude,
          lng: location.longitude,
          address: location.address
        });
        
        // Update the selected location immediately - map will update via useEffect
        setSelectedLocation({
      ...
      console.log('❌ [LocationSelector] GPS detection failed:', err.message);
    } finally {
      ...
      <div className="flex-1 relative">
        {selectedLocation && (
          <LocationMapNative
            ...
            onCurrentLocationClick={handleAutoDetect}
            currentLocationLoading={isDetecting}
          />
        )}
      </div>
