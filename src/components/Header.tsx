...
import { ArrowLeft, Home, PlusCircle, User, Shield, List, MapPin, Search, Menu, MessageCircle, ChevronDown, Heart, Briefcase, Package, Bell, Sparkles, Store } from 'lucide-react';
...
interface HeaderProps {
  ...
  onNavigate?: (screen: 'home' | 'create' | 'profile' | 'admin' | 'listing' | 'edit' | 'chat' | 'about' | 'safety' | 'terms' | 'privacy' | 'contact' | 'diagnostic' | 'create-wish' | 'create-task' | 'tasks' | 'wishes' | 'marketplace' | 'professionals' | 'shops') => void;
  isLoggedIn?: boolean;
  isAdmin?: boolean;
...
export function Header({
  ...
  const isMainScreen = ['home', 'create', 'profile', 'admin', 'chat', 'tasks', 'wishes', 'marketplace', 'professionals', 'shops'].includes(currentScreen);
  ...
  return (
    <header className="sticky top-0 z-50 bg-white border-b border-gray-200">
            ...
            {!isHomeScreen && isLoggedIn && (
              <>
                <button
                  ...
                  className="flex items-center gap-1.5 px-2 py-1.5 border-2 border-black text-black rounded-lg hover:bg-gray-50 transition-all text-xs font-medium whitespace-nowrap"
                >
      ...
      {/* Secondary Navigation Bar - Web View Only (Home, Tasks, Wishes, Marketplace, Professionals) */}
      {isMainScreen && onNavigate && (
