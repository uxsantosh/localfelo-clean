    ...
    try {
      ...
      if (updateError) {
        ...
        if (updateError.code === 'PGRST204' || updateError.code === '42703') {
          ...
          if (retryError) {
            console.error('❌ ❌ ❌ [useLocation] DATABASE UPDATE FAILED - CRITICAL DEBUG INFO:');
            ...
            console.error('Full Error Object:', JSON.stringify(retryError, null, 2));
            console.error('User ID:', userId);
            console.error('Attempting to save:', {
              city_id: newLocation.cityId,
              city: newLocation.city,
              area_id: newLocation.areaId,
              area: newLocation.area,
              latitude: finalLat,
              longitude: finalLon,
            });
            
            // ✅ Update UI silently - NO error message to user
            ...
            localStorage.setItem('localfelo_user_location_fallback', JSON.stringify(updatedLocation));
            localStorage.setItem('localfelo_location_version', Date.now().toString());
            window.dispatchEvent(new CustomEvent('locationChanged', { detail: updatedLocation }));
            
            setLocation(updatedLocation);
            setError(null); // ✅ NO ERROR TO USER
            setLoading(false);
            return;
          }
        ...
        // ✅ FIX: Handle 409 Conflict error (RLS policy or concurrent update)
        if (updateError.code === '409' || updateError.code === 'PGRST116') {
          console.error('❌ [useLocation] 409 Conflict - RLS policy or concurrent update issue:', updateError);
          console.error('❌ [useLocation] Error details:', JSON.stringify(updateError, null, 2));
          ...
          if (retryError) {
            console.error('❌ [useLocation] Retry after 409 also failed:', retryError);
            
            // ✅ Update local state anyway - NO ERROR TO USER
            const updatedLocation = {
              ...newLocation,
              latitude: finalLat,
              longitude: finalLon,
          ...
          console.log('✅ [useLocation] Location saved after retry');
          const updatedLocation = {
            ...newLocation,
            latitude: finalLat,
            longitude: finalLon,
      ...
      console.log('📍 [useLocation] Dispatching locationChanged event with:', { 
      ...
      setLocation(updatedLocation);
      console.log('✅ [useLocation] Location state updated');
      setLoading(false);
    } catch (err) {
      console.error('❌ [useLocation] Exception:', err);
      setError('Failed to update location');
