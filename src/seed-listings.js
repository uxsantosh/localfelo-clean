 ...
 * 
 * Run with: node seed-listings.js
 * 
 * This script:
 * 1. Fetches real product images from online sources
 * 2. Uploads them to Supabase Storage
 * 3. Creates realistic listings for all categories/cities/areas
 */
...
// Using public domain / CC0 images from various sources
  ...
  'Mobiles': [
    {
      ...
      price: 32000,
      images: [
        'https://images.unsplash.com/photo-1603791239531-49180c485003?w=800&q=80', // iPhone
...
async function seedListings() {
  ...
  // 4. Get a demo owner token (use any existing profile or create a seed profile)
  ...
  // 5. Create listings for each category in each city (first area only)
  for (const category of categories) {
    ...
    // Create listings in first 2-3 cities only (to avoid too many listings)
    const citiesToUse = cities.slice(0, 3);

    for (const city of citiesToUse) {
      // Get first area for this city
      const cityAreas = areas.filter(a => a.city_id === city.id);
      if (cityAreas.length === 0) continue;

      const area = cityAreas[0]; // Use first area only

      // Pick a random listing from sample data
